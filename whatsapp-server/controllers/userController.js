...
const signup = async (req, res) => {
    ...
    if (existingUser) {
        return apiHelper.sendError(res,
            "User already existed. Please login instead", 402)
    }
    ...
    try {
        ...
        await mailer.mailTransport().sendMail({
            ...
            html: mailer.generateEmailTemplate(user.fullName, OTP, "Sign up")
        }, function (error, info) {
            if (error) {
                console.log(error)
            } else {
        ...
        return apiHelper.sendSuccessResponse(res, "Signup successfully",
            {user}, 201)
    } catch (e) {
        console.log(e)
...
const login = async (req, res) => {
    ...
    if (!existingUser) {
        return apiHelper.sendError(res, "User does not exist. Please signup", 400)
    }
...
const forgotPassword = async (req, res) => {
    ...
    await mailer.mailTransport().sendMail({
        ...
        html: mailer.generateEmailTemplate(user.fullName,
            randomOTP, "Reset password")
    }, function (error, info) {
        if (error) {
            console.log(error)
        } else {
    ...
    return apiHelper.sendSuccessResponse(res, 'Password reset otp ' +
...
const verifyOTPResetPass = async (req, res) => {
    ...
    return apiHelper.sendSuccessResponse(res, 'Verified successfully', {user})
...
const resetPassword = async (req, res) => {
    ...
    if (!isPassValid) {
        return apiHelper.sendError(res, 'password is invalid, 8-24 characters, ' +
